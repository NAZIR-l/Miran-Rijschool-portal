...
export default defineComponent({
  ...
  setup() {
    ...
    // Function to get translated traffic sign names
    const getTrafficSignName = (signId) => {
      return t(`traffic.signs.${signId}`) || signId;
    };

    // Enhance titles from mapping if present
    groups.value.forEach((g) => {
      g.items.forEach((it) => {
        const translatedName = getTrafficSignName(it.id);
        if (translatedName !== it.id) it.title = translatedName;
      });
    });
